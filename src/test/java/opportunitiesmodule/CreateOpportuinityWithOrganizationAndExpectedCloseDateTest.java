...
public class CreateOpportuinityWithOrganizationAndExpectedCloseDateTest {
	...
	public void creatingOpportunityWithOrganization() throws IOException, InterruptedException {
		...
		//to get expected close date
				...
				cal.add(Calendar.DAY_OF_MONTH,+5);//after 5 days
				
				//to get that date
				
				String closeDate = sim.format(cal.getTime());
				
				WebElement ele1 = driver.findElement(By.xpath("//input[@name='closingdate']"));
				ele1.clear();
				ele1.sendKeys(closeDate);
		...
		String closedateinfo = driver.findElement(By.xpath("(//td[@class='dvtCellInfo'])[6]")).getText();
		if(closedateinfo.contains(closeDate))
		{
			System.out.println(closeDate+ " is created == PASS");

		}
		else {
			System.out.println(closeDate + " is not created == Fail");
